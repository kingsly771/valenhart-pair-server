/**
 * VALENHART MD - A WhatsApp Bot
 * Copyright (c) 2024 ꧁ꪾᴍʀ͢.SULTAN LUCIEN VALENHART꧂
 * 
...
// human-readable replaced with inline CJS-compatible implementation
function sizeFormatter(opts = {}) {
    const std = opts.std || 'JEDEC';
    const dp = opts.decimalPlaces != null ? opts.decimalPlaces : 2;
    const render = opts.render || ((val, sym) => `${val} ${sym}B`);
    const units = std === 'IEC'
        ? ['', 'Ki', 'Mi', 'Gi', 'Ti', 'Pi', 'Ei', 'Zi', 'Yi']
        : ['', 'K', 'M', 'G', 'T', 'P', 'E', 'Z', 'Y'];
    const base = std === 'IEC' ? 1024 : 1000;
    return (bytes) => {
        if (bytes === 0) return render('0', '');
        let i = 0;
        let val = Math.abs(bytes);
        while (val >= base && i < units.length - 1) { val /= base; i++; }
        const str = opts.keepTrailingZeroes
            ? val.toFixed(dp)
            : parseFloat(val.toFixed(dp)).toString();
        return render(str, units[i]);
    };
}
